...
class TT800
	...
	def nextInt
		...
		if(@i == @N)
			k = 0
			while(k < @N - @M)
				@mt[k] = @mt[k+@M] ^ (@mt[k] >> 1) ^ @mag[@mt[k] % 2]
				k = k + 1
			end
			while(k < @N)
				@mt[k] = @mt[k + (@M-@N)] ^ (@mt[k] >> 1) ^ @mag[@mt[k] % 2]
				k = k + 1
			end
			@i = 0
		end
...
for i in 0 ... 100
	...
	fm2.write(complement(tt800.nextInt))	# nextIntで生成した乱数を32bitに補完した後 ファイルに書き込み
	fl.write(complement(lfsr.nextInt))
end

#for i in 0 ... 50
#	puts(complement(tt800.nextInt))
#end
